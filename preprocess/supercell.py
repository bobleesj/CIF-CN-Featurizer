...
def get_coords_list(block, loop_values):
    ...
    for i in range(loop_length):
        ...
        all_coords = get_coords_after_sym_op(
            block,
            float(atom_site_x),
            float(atom_site_y),
            float(atom_site_z),
...
def get_coords_after_sym_op(
    ...
    if block.find_loop("_space_group_symop_operation_xyz"):
        symmetry_operation_loop = block.find_loop(
    ...
    if not symmetry_operation_loop:
        print(
            "Neither '_symmetry_equiv_pos_as_xyz' nor '_space_group_symop_operation_xyz' were found in the block."
        )
        return []
        ...
        for j, point2 in enumerate(flattened_points):
            if i != j:
                ...
                if (
                    ...
                    distance, label1, label2 = calculate_distance(
...
def filter_and_swap_pairs(atomic_pair_list, label):
    ...
    """
    filtered_atomic_pair_list = [
        pair for pair in atomic_pair_list if label in pair["labels"]
    ]

    for j in range(len(filtered_atomic_pair_list)):
        filtered_atomic_pair_list[j] = swap_labels_and_coordinates(
            filtered_atomic_pair_list[j], label
        )
...
def find_most_common_distances_and_points(
    filtered_atomic_pair_list, label, unique_distances
):
    """
...
def find_common_points(points_with_most_min_distances):
    ...
    """
    set_of_min_distances = [
        set(points) for points in points_with_most_min_distances
    ]
...
def get_atom_pair_info_dict(unique_atom_labels, atomic_pairs):
    ...
    for idx, atom_label in enumerate(unique_atom_labels):
        ...
        (
            most_common_distance_counts,
            points_with_common_distances,
        ) = find_most_common_distances_and_points(
        ...
        atom_pairs_related_to_point = [
            ...
            if pair["point_pair"][0] == point_with_most_common_distances[0]
            or pair["point_pair"][1] == point_with_most_common_distances[0]
        ]

        atom_pairs_related_to_point_sorted = sorted(
            atom_pairs_related_to_point, key=lambda x: x[2]
        )

        atom_pairs_info_dict.setdefault(
            atom_label,
            {
...
def get_atomic_pair_list_dict(
    all_points, unique_labels, cell_lengths, cell_angles_rad
):
    """
