...
def get_unit_cell_volume(lenghts, angles):
    ...
    volume = (
        ...
        * sympy.sqrt(
            1
            - sympy.cos(alpha_angle) ** 2
            - sympy.cos(beta_angle) ** 2
            - sympy.cos(gamma_angle) ** 2
            + 2
            * sympy.cos(alpha_angle)
            * sympy.cos(beta_angle)
            * sympy.cos(gamma_angle)
        )
    )
    return volume
...
def compute_packing_efficiency(
    atoms, CIF_loop_values, CIF_rad_refined_dict, cell_lengths, cell_angles_rad
):
    ...
    for atom_site_type, atom_site_symmetry_multiplicity in zip(
        atom_site_types, atom_site_symmetry_multiplicities
    ):
    ...
    for atom, count in atom_counts.items():
        total_volume += (
            count * (4 / 3) * math.pi * CIF_rad_refined_dict[atom] ** 3
        )
