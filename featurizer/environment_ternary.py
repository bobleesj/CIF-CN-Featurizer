...
def get_shortest_dist_count_with_tol(
    ...
    for label, atom_dict in atom_pairs_info_dict.items():
        ...
        if label in unique_labels:
            ...
            shortest_distances_pair = [
                info[2] for info in atom_dict["pair_info"]
            ]
    ...
    for label, atom_dict in atom_pairs_info_dict.items():
        ...
        if label in unique_shortest_labels:
            ...
            count_within_tolerance = atom_dict[
                "count_within_5_percent_of_shortest"
            ]
...
def get_second_by_first_shortest_dist(
    ...
    for label in unique_shortest_labels:
        if label in atom_pairs_info_dict:
            ...
            second_by_first_shortest_dist = (
                second_shortest_dist / first_shortest_dist
            )
    ...
    for atom_type in sum_ratios.keys():
        if label_counts[atom_type] > 0:  # Avoid division by zero
            average_ratios[atom_type] = (
                sum_ratios[atom_type] / label_counts[atom_type]
            )
        else:
...
def get_second_shortest_dist_count(
    ...
    for label in unique_shortest_labels:
        if label in atom_pairs_info_dict:
            ...
            if label.startswith(R):
                R_second_shortest_dist_count = data_dict[
                    "shortest_distances_count"
                ][1]
            elif label.startswith(M):
                M_second_shortest_dist_count = data_dict[
                    "shortest_distances_count"
                ][1]
            elif label.startswith(X):
                X_second_shortest_dist_count = data_dict[
                    "shortest_distances_count"
                ][1]
    ...
    for label in unique_labels:
        ...
        # Sum up the shortest_distances_count for this atom type
        sum_second_shortest_counts[atom_type] += data_dict[
            "shortest_distances_count"
        ][1]
...
def get_homoatomic_dist_by_shortest_dist_countd(
    ...
    R_avg_homoatomic_dist_by_shortest_dist = (
        R_total_dist_by_shortest_dist / atom_counts[R]
        if atom_counts[R] > 0
        else 0
    )
    M_avg_homoatomic_dist_by_shortest_dist = (
        M_total_dist_by_shortest_dist / atom_counts[M]
        if atom_counts[M] > 0
        else 0
    )
    X_avg_homoatomic_dist_by_shortest_dist = (
        X_total_dist_by_shortest_dist / atom_counts[X]
        if atom_counts[X] > 0
        else 0
    )
...
def get_R_X_X_count_at_shortest_dist(
    ...
    R_avg_new_count_at_R_shortest_dist = (
        R_new_count_at_R_shortest_dist / atom_counts[R]
        if atom_counts[R] > 0
        else 0
    )
    M_avg_new_count_at_R_shortest_dist = (
        M_new_count_at_R_shortest_dist / atom_counts[R]
        if atom_counts[R] > 0
        else 0
    )
    X_avg_new_count_at_R_shortest_dist = (
        X_new_count_at_R_shortest_dist / atom_counts[R]
        if atom_counts[R] > 0
        else 0
    )

    R_avg_new_count_at_M_shortest_dist = (
        R_new_count_at_M_shortest_dist / atom_counts[M]
        if atom_counts[M] > 0
        else 0
    )
    M_avg_new_count_at_M_shortest_dist = (
        M_new_count_at_M_shortest_dist / atom_counts[M]
        if atom_counts[M] > 0
        else 0
    )
    X_avg_new_count_at_M_shortest_dist = (
        X_new_count_at_M_shortest_dist / atom_counts[M]
        if atom_counts[M] > 0
        else 0
    )

    R_avg_new_count_at_X_shortest_dist = (
        R_new_count_at_X_shortest_dist / atom_counts[X]
        if atom_counts[X] > 0
        else 0
    )
    M_avg_new_count_at_X_shortest_dist = (
        M_new_count_at_X_shortest_dist / atom_counts[X]
        if atom_counts[X] > 0
        else 0
    )
    X_avg_new_count_at_X_shortest_dist = (
        X_new_count_at_X_shortest_dist / atom_counts[X]
        if atom_counts[X] > 0
        else 0
    )
