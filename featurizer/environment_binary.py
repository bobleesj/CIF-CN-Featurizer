...
def get_unique_shortest_labels(atom_pairs_info_dict, unique_labels, cif_parser):
    ...
    if multiple_labels:
        for multiple_label in multiple_labels:
            ...
            matching_labels = [
                label
                for label in atom_pairs_info_dict.keys()
            ...
            for label in matching_labels:
                ...
                current_shortest_distance = min(
                    atom_pairs_info_dict[label]["shortest_distances"]
                )
...
def get_shortest_dist_count_with_tol(
    ...
    for label, atom_dict in atom_pairs_info_dict.items():
        if label in unique_labels:
            ...
            atom_type = cif_parser.get_atom_type(
                label
            )  # Assuming cif_parser.get_atom_type() can handle your label format
...
def get_second_shortest_dist_count(
    ...
    for label in unique_shortest_labels:
        if label in atom_pairs_info_dict:
            ...
            if label.startswith(A):
                A_second_shortest_dist_count = data_dict[
                    "shortest_distances_count"
                ][1]
            elif label.startswith(B):
                B_second_shortest_dist_count = data_dict[
                    "shortest_distances_count"
                ][1]
...
def get_homoatomic_dist_by_shortest_dist_count(
    unique_atoms_tuple,
    unique_labels,
    unique_shortest_labels,
    atom_pairs_info_dict,
    ...
    for label in unique_shortest_labels:
        if label in atom_pairs_info_dict:
            ...
            for pair in pair_info:
                ...
                if label.startswith(A) and (
                    first_label.startswith(A) and second_label.startswith(A)
                ):
                    A_homoatomic_dist_by_shortest_dist = dist / shortest_dist
                    break
                elif label.startswith(B) and (
                    first_label.startswith(B) and second_label.startswith(B)
                ):
    ...
    A_avg_homoatomic_dist_by_shortest_dist = (
        A_total_dist_by_shortest_dist / atom_counts[A]
        if atom_counts[A] > 0
        else 0
    )
    B_avg_homoatomic_dist_by_shortest_dist = (
        B_total_dist_by_shortest_dist / atom_counts[B]
        if atom_counts[B] > 0
        else 0
    )
...
def get_A_B_count_at_shortest_dist(
    unique_atoms_tuple,
    unique_labels,
    unique_shortest_labels,
    atom_pairs_info_dict,
    ...
    A_avg_new_count_at_A_shortest_dist = (
        A_new_count_at_A_shortest_dist / atom_counts[A]
        if atom_counts[A] > 0
        else 0
    )
    B_avg_new_count_at_A_shortest_dist = (
        B_new_count_at_A_shortest_dist / atom_counts[A]
        if atom_counts[A] > 0
        else 0
    )

    A_avg_new_count_at_B_shortest_dist = (
        A_new_count_at_B_shortest_dist / atom_counts[B]
        if atom_counts[B] > 0
        else 0
    )
    B_avg_new_count_at_B_shortest_dist = (
        B_new_count_at_B_shortest_dist / atom_counts[B]
        if atom_counts[B] > 0
        else 0
    )
    ...
    return (
        ...
        B_new_count_at_B_shortest_dist,
    ), (
        A_avg_new_count_at_A_shortest_dist,
        B_avg_new_count_at_A_shortest_dist,
        A_avg_new_count_at_B_shortest_dist,
