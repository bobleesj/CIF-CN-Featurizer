...
def get_atom_type_prefix(atom_type, A, B, R, M, X):
    ...
    """
    atom_type_map = {R: "R", M: "M", X: "X", A: "A", B: "B"}
...
def get_combined_prefix(first_label, second_label, A, B, R, M, X):
    ...
    first_prefix = get_atom_type_prefix(
        get_atom_type(first_label), A, B, R, M, X
    )
    second_prefix = get_atom_type_prefix(
        get_atom_type(second_label), A, B, R, M, X
    )
        ...
        pairs_and_radius_sums.append(
            {
                ...
                "CIF_rad_refined_sum": rad_sum["CIF_rad_refined_sum"].get(
                    prefix, 0.0
                ),
...
def calculate_diff_counts_per_label(
    unique_labels, output_dict, rad_sum, A, B, R, M, X
):
    """
    ...
    for label in unique_labels:
        ...
        if label in output_dict:
            (
                shortest_dist,
                pairs_and_radius_sums,
            ) = get_pair_info_and_radius_sums(
                label, output_dict[label], rad_sum, A, B, R, M, X
            )

            for pair_info in pairs_and_radius_sums:
                dist_sums = {
                    "dist_by_shortest_dist": pair_info["pair"][2]
                    / shortest_dist,
                    "dist_by_CIF_rad_sum": pair_info["pair"][2]
                    / pair_info["CIF_rad_sum"],
                    "dist_by_CIF_rad_refined_sum": pair_info["pair"][2]
                    / pair_info["CIF_rad_refined_sum"],
                    "dist_by_Pauling_rad_sum": pair_info["pair"][2]
                    / pair_info["Pauling_rad_sum"],
                }

                prev_dist_sums = update_difference_lists(
                    pair_info["pair"][2],
                    dist_sums,
                    prev_dist_sums,
                    diff_lists,
...
def get_pair_info_for_label(label, atom_pair_info_dict):
    ...
    """
    return (
        atom_pair_info_dict[label]["pair_info"]
        if label in atom_pair_info_dict
        else None
    )
...
def find_most_common_point(pair_info, counts, label, dist_type):
    ...
    """
    ith_point_number = [
        pair[0][0] for pair in pair_info[: counts[label][dist_type]]
    ]
...
def convert_to_cartesian(pair, cell_lengths, cell_angles_rad):
    ...
    """
    cart_coords_ith = get_cartesian_from_fractional(
        pair[3][0], cell_lengths, cell_angles_rad
    )
    cart_coords_jth = get_cartesian_from_fractional(
        pair[3][1], cell_lengths, cell_angles_rad
    )

    return cart_coords_ith, cart_coords_jth


def process_atom_pairs(
    isBinary,
    pair_info,
    cell_lengths,
    cell_angles_rad,
    ...
    for pair in supercell.swap_pairs(
        pair_info, counts, label, dist_type, most_common_point
    ):
        cart_coords_ith, cart_coords_jth = convert_to_cartesian(
            pair, cell_lengths, cell_angles_rad
        )
...
def process_labels(
    ...
    if pair_info and label in counts:
        for dist_type in counts[label]:
            most_common_point = find_most_common_point(
                pair_info, counts, label, dist_type
            )
