...
def get_env_wychoff_binary_df(
    ...
    radii, _ = optimize.optimize_CIF_rad_binary(
    ...
    min_wyckoff_multiplicity = min(
        A_lowest_wyckoff_multiplicity, B_lowest_wyckoff_multiplicity
    )
    ...
    atomic_environment_binary_Wyckoff_data = {
        ...
        "identical_lowest_wyckoff_count": [
            identical_lowest_wyckoff_multiplicity_count
        ],
    ...
    atomic_environment_wyckoff_binary_df = pd.concat(
        [atomic_environment_wyckoff_binary_df, df], ignore_index=True
    )
    ...
    for property in [
        ...
        highest_property_element = (
            A if A_property_value > B_property_value else B
        )
        lowest_property_element = (
            A if A_property_value < B_property_value else B
        )
        ...
        for element in lowest_wyckoff_elements:
            ...
            property_value = (
                A_property_value if element == A else B_property_value
            )
    ...
    atomic_environment_wyckoff_universal_df = pd.concat(
        [atomic_environment_wyckoff_universal_df, df], ignore_index=True
    )
    atomic_environment_wyckoff_universal_df = (
        atomic_environment_wyckoff_universal_df.round(5)
    )
    ...
    for column in atomic_environment_wyckoff_binary_df.columns:
        atomic_environment_wyckoff_binary_df[
            column
        ] = atomic_environment_wyckoff_binary_df[column].apply(
...
def get_env_wychoff_ternary_df(
    filename,
    xl,
    ...
    radii, _ = optimize.optimize_CIF_rad_ternary(
    ...
    (
        ...
        M_info,
        X_info,
    ) = env_wychoff_featurizer.get_atomic_environment_in_ternary(
        CIF_loop_values, R, M, X
    )
    ...
    min_wyckoff_multiplicity = min(
        ...
        X_lowest_wyckoff__multiplicity,
    )
    ...
    atomic_environment_wyckoff_ternary_df = pd.concat(
        [atomic_environment_wyckoff_ternary_df, df], ignore_index=True
    )
    ...
    for column in atomic_environment_wyckoff_ternary_df.columns:
        atomic_environment_wyckoff_ternary_df[
            column
        ] = atomic_environment_wyckoff_ternary_df[column].apply(
            lambda x: ", ".join(map(str, x)) if isinstance(x, list) else x
        )
