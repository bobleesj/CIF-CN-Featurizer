...
def get_interatomic_binary_df(
    ...
    (
        ...
        True, unique_atoms_tuple, atomic_pair_list
    )
    ...
    radii, obj_value = optimize.optimize_CIF_rad_binary(
        A_CIF_rad, B_CIF_rad, shortest_distances_pair, True
    )
    ...
    interatomic_A_A_minus_ref_diff = (
        interatomic_distA_A - A_A_radius_sum
    ) / interatomic_distA_A
    interatomic_B_B_minus_ref_diff = (
        interatomic_distB_B - B_B_radius_sum
    ) / interatomic_distB_B
    interatomic_A_B_minus_ref_diff = (
        interatomic_distA_B - A_B_radius_sum
    ) / interatomic_distA_B

    atoms = (A, B)
    CIF_rad_refined_dict = {A: A_CIF_rad_refined, B: B_CIF_rad_refined}
    packing_efficiency = unitcell.compute_packing_efficiency(
        atoms,
        CIF_loop_values,
    ...
    interatomic_binary_df = pd.concat(
        [interatomic_binary_df, df], ignore_index=True
    )
    ...
    homoatomic_distances = {
        key: shortest_distances_pair[key] for key in ["AA", "BB"]
    }
    ...
    interatomic_universal_df = pd.concat(
        [interatomic_universal_df, df], ignore_index=True
    )
...
def get_interatomic_ternary_df(
    ...
    (
        ...
        False, unique_atoms_tuple, atomic_pair_list
    )
    ...
    radii, obj_value = optimize.optimize_CIF_rad_ternary(
        R_CIF_rad, M_CIF_rad, X_CIF_rad, shortest_distances_pair, True
    )
    ...
    interatomic_R_R_minus_ref_diff = (
        shortest_RR - R_R_radius_sum
    ) / shortest_RR
    interatomic_M_M_minus_ref_diff = (
        shortest_MM - M_M_radius_sum
    ) / shortest_MM
    interatomic_X_X_minus_ref_diff = (
        shortest_XX - X_X_radius_sum
    ) / shortest_XX
    interatomic_R_M_minus_ref_diff = (
        shortest_RM - R_M_radius_sum
    ) / shortest_RM
    interatomic_R_X_minus_ref_diff = (
        shortest_RX - R_X_radius_sum
    ) / shortest_RX
    interatomic_M_X_minus_ref_diff = (
        shortest_MX - M_X_radius_sum
    ) / shortest_MX
    ...
    interatomic_ternary_df = pd.concat(
        [interatomic_ternary_df, df], ignore_index=True
    )
    interatomic_ternary_df = interatomic_ternary_df.round(5)

    # Get the shortest homo/hetroatomic distance
    homoatomic_distances = {
        key: shortest_distances_pair[key] for key in ["RR", "MM", "XX"]
    }
    heteroatomic_distances = {
        key: shortest_distances_pair[key] for key in ["RM", "MX", "RX"]
    }
    shortest_homoatomic_distance = min(homoatomic_distances.values())
