...
def run_main():
    ...
    file_path_list = [
        os.path.join(cif_folder_dir, file)
        for file in os.listdir(cif_folder_dir)
    ...
    for idx, filename in enumerate(file_path_list, start=1):
        ...
        if isBinary:
            ...
            ) = interatomic_featurizer.get_interatomic_binary_df(
                filename,
                interatomic_binary_df,
                interatomic_universal_df,
                all_points,
        ...
        if isTernary:
            (
                ...
                interatomic_universal_df,
            ) = interatomic_featurizer.get_interatomic_ternary_df(
                interatomic_ternary_df,
                interatomic_universal_df,
                unique_atoms_tuple,
                atomic_pair_list,
        ...
        click.echo(
            style(
                f"{execution_time:.2f}s to process {len(all_points)} atoms (total time {running_total_time:.2f}s)",
                fg="green",
            )
        )
    ...
    if num_files_processed != 0:
        ...
        save_to_csv_directory(
            cif_folder_dir, round_df(featurizer_log_df), "featurizer_log"
        )
