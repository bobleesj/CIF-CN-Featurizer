import numpy as np
from core.preprocess.cif_parser import *
from core.utils.unit import *
                    ...
                    if rounded_distance(pair["distance"]) == rounded_distance(dist):
                        for point, point_label in sorted_pair:
                            if point_label == label:
                                new_count = (
                                    point_label_counts[idx].get((point, point_label), 0)
                                    + 1
                                )
                                point_label_counts[idx][(point, point_label)] = (
                                    new_count
                                )
