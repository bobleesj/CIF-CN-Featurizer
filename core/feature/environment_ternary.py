...
def get_shortest_distances_count(
    unique_atoms_tuple,
    unique_labels,
...
def get_shortest_dist_count_with_tol(
    unique_atoms_tuple,
    unique_labels,
    unique_shortest_labels,
    atom_pairs_info_dict,
    ...
    R_shortest_tol_dist_count = M_shortest_tol_dist_count = (
    ...
    for label, atom_dict in atom_pairs_info_dict.items():
        ...
        if label in unique_labels:
            ...
            count_within_tolerance = sum(
                1
                for distance in shortest_distances_pair
    ...
    for label in unique_labels:
        ...
        sum_counts_within_tol[atom_type] += data_dict[
            "count_within_5_percent_of_shortest"
        ]
...
def get_second_shortest_dist_count(
    ...
    for label in unique_labels:
        ...
        # Sum up the shortest_distances_count for this atom type
        sum_second_shortest_counts[atom_type] += data_dict["shortest_distances_count"][
            1
        ]
...
def get_homoatomic_dist_by_shortest_dist_countd(
    ...
    for label in unique_shortest_labels:
        if label in atom_pairs_info_dict:
            ...
            for pair in pair_info:
                ...
                if label.startswith(R) and (
                    first_label.startswith(R) and second_label.startswith(R)
                ):
                    R_homoatomic_dist_by_shortest_dist = dist / shortest_dist
                    break
                elif label.startswith(M) and (
                    first_label.startswith(M) and second_label.startswith(M)
                ):
                    M_homoatomic_dist_by_shortest_dist = dist / shortest_dist
                    break
                elif label.startswith(X) and (
                    first_label.startswith(X) and second_label.startswith(X)
                ):
    ...
    R_avg_homoatomic_dist_by_shortest_dist = (
        R_total_dist_by_shortest_dist / atom_counts[R] if atom_counts[R] > 0 else 0
    )
    M_avg_homoatomic_dist_by_shortest_dist = (
        M_total_dist_by_shortest_dist / atom_counts[M] if atom_counts[M] > 0 else 0
    )
    X_avg_homoatomic_dist_by_shortest_dist = (
        X_total_dist_by_shortest_dist / atom_counts[X] if atom_counts[X] > 0 else 0
    )
...
def get_R_X_X_count_at_shortest_dist(
    ...
    R_avg_new_count_at_R_shortest_dist = (
        R_new_count_at_R_shortest_dist / atom_counts[R] if atom_counts[R] > 0 else 0
    )
    M_avg_new_count_at_R_shortest_dist = (
        M_new_count_at_R_shortest_dist / atom_counts[R] if atom_counts[R] > 0 else 0
    )
    X_avg_new_count_at_R_shortest_dist = (
        X_new_count_at_R_shortest_dist / atom_counts[R] if atom_counts[R] > 0 else 0
    )

    R_avg_new_count_at_M_shortest_dist = (
        R_new_count_at_M_shortest_dist / atom_counts[M] if atom_counts[M] > 0 else 0
    )
    M_avg_new_count_at_M_shortest_dist = (
        M_new_count_at_M_shortest_dist / atom_counts[M] if atom_counts[M] > 0 else 0
    )
    X_avg_new_count_at_M_shortest_dist = (
        X_new_count_at_M_shortest_dist / atom_counts[M] if atom_counts[M] > 0 else 0
    )

    R_avg_new_count_at_X_shortest_dist = (
        R_new_count_at_X_shortest_dist / atom_counts[X] if atom_counts[X] > 0 else 0
    )
    M_avg_new_count_at_X_shortest_dist = (
        M_new_count_at_X_shortest_dist / atom_counts[X] if atom_counts[X] > 0 else 0
    )
    X_avg_new_count_at_X_shortest_dist = (
        X_new_count_at_X_shortest_dist / atom_counts[X] if atom_counts[X] > 0 else 0
    )
