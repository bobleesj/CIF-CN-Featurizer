...
def get_unique_shortest_labels(atom_pairs_info_dict, unique_labels, cif_parser):
    ...
    if multiple_labels:
        for multiple_label in multiple_labels:
            ...
            matching_labels = [
                label for label in unique_labels if label.startswith(multiple_label)
            ]
            unique_shortest_labels = [
...
def get_homoatomic_dist_by_shortest_dist_count(
    ...
    A_avg_homoatomic_dist_by_shortest_dist = (
        A_total_dist_by_shortest_dist / atom_counts[A] if atom_counts[A] > 0 else 0
    )
    B_avg_homoatomic_dist_by_shortest_dist = (
        B_total_dist_by_shortest_dist / atom_counts[B] if atom_counts[B] > 0 else 0
    )
...
def get_A_B_count_at_shortest_dist(
    ...
    A_avg_new_count_at_A_shortest_dist = (
        A_new_count_at_A_shortest_dist / atom_counts[A] if atom_counts[A] > 0 else 0
    )
    B_avg_new_count_at_A_shortest_dist = (
        B_new_count_at_A_shortest_dist / atom_counts[A] if atom_counts[A] > 0 else 0
    )

    A_avg_new_count_at_B_shortest_dist = (
        A_new_count_at_B_shortest_dist / atom_counts[B] if atom_counts[B] > 0 else 0
    )
    B_avg_new_count_at_B_shortest_dist = (
        B_new_count_at_B_shortest_dist / atom_counts[B] if atom_counts[B] > 0 else 0
    )
