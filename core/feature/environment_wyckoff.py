...
import core.utils.data as data
import core.preprocess.optimize as optimize
import core.feature.distance as distance
...
def get_env_wychoff_binary_df(
    ...
    for column in atomic_environment_wyckoff_binary_df.columns:
        atomic_environment_wyckoff_binary_df[column] = (
            atomic_environment_wyckoff_binary_df[
                column
...
def get_env_wychoff_ternary_df(
    ...
    for column in atomic_environment_wyckoff_ternary_df.columns:
        atomic_environment_wyckoff_ternary_df[column] = (
            atomic_environment_wyckoff_ternary_df[
                column
            ].apply(lambda x: ", ".join(map(str, x)) if isinstance(x, list) else x)
        )
